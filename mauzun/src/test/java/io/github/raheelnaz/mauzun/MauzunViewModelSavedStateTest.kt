...
private class SaveableViewModel : MauzunViewModel<Int, Int, Nothing>() {
    @Composable
...
private class FiltersViewModel : MauzunViewModel<String, Filters, Nothing>() {
    @Composable
...
private class TwoFieldViewModel : MauzunViewModel<Int, Pair<Int, String>, Nothing>() {
    @Composable
...
private class OpaqueViewModel : MauzunViewModel<Int, Int, Nothing>() {
    @Composable
    override fun present(events: Flow<Int>): Int {
...
private class LambdaViewModel : MauzunViewModel<Int, Int, Nothing>() {
    @Composable
    override fun present(events: Flow<Int>): Int {
...
private class SelfRetrievingViewModel(
    ...
    @Composable
    override fun present(events: Flow<Int>): Int {
        mauzunViewModel<SelfRetrievingViewModel>(
            key = "self",
...
private abstract class SwappableViewModel : MauzunViewModel<Int, Int, Nothing>() {
    @Composable
    override fun present(events: Flow<Int>): Int {
        var value by rememberSaveable { mutableIntStateOf(0) }
        CollectEvents(events) { value = it }
...
private class ArgViewModel(val arg: String) : MauzunViewModel<Int, Int, Nothing>() {
    @Composable
...
private class Host(
    ...
    fun <VM : MauzunViewModel<*, *, *>> obtainWithoutMauzun(
        key: String,
        modelClass: Class<VM>,
...
private suspend inline fun <reified VM : MauzunViewModel<*, *, *>> Host.obtain(
    key: String? = null,
...
private suspend inline fun <reified VM : MauzunViewModel<*, *, *>> Host.attach(
    viewModel: VM,
    key: String? = null,
): PresenterEntry<VM> = moleculeFlow(RecompositionMode.Immediate) {
    withLifecycle {
...
@Config(sdk = [34])
class MauzunViewModelSavedStateTest {
    ...
    @Test
    fun `the 0_9 saved state key remains stable`() {
        assertThat(PresenterSavedState.KEY)
            .isEqualTo("io.github.raheelnaz.molecule.presenter-saved-state.v1")
    }
    ...
    fun `an existing unstarted ViewModel can opt into saved state`() = runTest(dispatcher) {
        val host = Host()
        val existing = host.obtainWithoutMauzun(
            key = "counter",
    ...
    fun `late attachment fails instead of silently losing restoration`() = runTest(dispatcher) {
        val host = Host()
        val viewModel = host.obtainWithoutMauzun(
            key = "counter",
            modelClass = SaveableViewModel::class.java,
            factory = saveableFactory,
        )
        ...
        assertFailure { host.obtain<SaveableViewModel>("counter", saveableFactory) }
            ...
            .hasMessage(
                "rememberSaveable state was attached after the presenter started; " +
                    "obtain the presenter with mauzunViewModel() before its binding starts",
            )
        host.viewModelStore.clear()
    }
    ...
    fun `retrieval from inside present is rejected`() = runTest(dispatcher) {
        ...
        assertFailure { viewModel.bindingInstance.state }
            .isInstanceOf(IllegalStateException::class)
            .hasMessage("mauzunViewModel() cannot be called from MauzunViewModel.present()")
        host.viewModelStore.clear()
    }
    ...
    fun `an owner without saved state support names the problem`() = runTest(dispatcher) {
        ...
        val outcome = runCatching {
            moleculeFlow(RecompositionMode.Immediate) {
                mauzunViewModel<SaveableViewModel>(
        ...
        val named = generateSequence(outcome.exceptionOrNull()) { it.cause }
            .firstOrNull { it.message?.startsWith("mauzunViewModel needs") == true }
        assertThat(named)
            .isNotNull()
            .isInstanceOf(IllegalStateException::class)
            .hasMessage(
