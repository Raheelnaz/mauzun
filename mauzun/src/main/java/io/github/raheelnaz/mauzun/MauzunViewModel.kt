...
public abstract class MauzunViewModel<Event : Any, Model : Any, Effect : Any> :
    ViewModel() {
    ...
    internal val bindingInstance: PresenterBinding<Event, Model, Effect> =
        object : PresenterBinding<Event, Model, Effect> {
            override val state: StateFlow<Model> get() = this@MauzunViewModel.state
            override val effects: Flow<Effect> get() = this@MauzunViewModel.effects
            override fun onEvent(event: Event) = this@MauzunViewModel.onEvent(event)
        }

    // mauzunViewModel() calls this before the first state read, and the shared lock keeps the
    // two from racing.
